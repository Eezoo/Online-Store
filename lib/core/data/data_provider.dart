...
class DataProvider extends ChangeNotifier {
  ...
  Future<void> refreshData() async {
    // إعادة جلب البيانات من الخادم وتحديث القوائم
    try {
      await Future.wait([
        getAllCategory(),
        getAllSubCategory(),
        ...
        getAllProduct(),
        getAllPoster(),
        getAllOrderByUser(null),
      ]);
    } catch (e) {
      // خطأ في إعادة الجلب: يمكن التعامل مع اللوغ أو إظهار رسالة لاحقًا
    }
    notifyListeners();
  }
  ...
  void filterCategorise(String keyword) {
    if (keyword.isEmpty) {
      _filteredCategories = List.from(_allCategories);
    } else {
  ...
  void filterSudCategorise(String keyword) {
    if (keyword.isEmpty) {
      _filteredSubCategories = List.from(_allSubCategories);
    } else {
      final lowerKeyword = keyword.toLowerCase();
