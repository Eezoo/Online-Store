...
import 'package:flutter_ecommerce_start/models/api_response.dart';
import 'package:flutter_ecommerce_start/utility/utility_extention.dart';
...
class CartProvider extends ChangeNotifier {
  ...
  //هنا الاضافات الجديده
  TextEditingController ditleController = TextEditingController();
  ...
  String selectedPaymentOption = 'الدفع نقداً'; //prepaid
  ...
  checkCoupon() async {
    try {
      ...
      List<String> productIds =
          myCartItems.map((cartItme) => cartItme.productId).toList();
      ...
      if (response.isOk) {
        ...
        if (apiResponse.success == true) {
          ...
          SnackBarHelper.showErrorSnackBar(
              'Invalid Coupon Code : ${apiResponse.message}');
        }
      } else {
        SnackBarHelper.showErrorSnackBar(
            'Eerror: ${response.body?['message'] ?? response.statusText}');
      }
      notifyListeners();
    } catch (e) {
      print(e);
      SnackBarHelper.showErrorSnackBar('An error: $e');
      rethrow;
    }
  }
  ...
  addOrder(BuildContext context) async {
    try {
      ...
      myCartItems = flutterCart.cartItemsList;
      final String userId = _userProvider.getLoginUsr()?.sId ?? '';

      if (userId.isEmpty) {
        SnackBarHelper.showErrorSnackBar('Please login to place an order');
        return;
      }

      if (myCartItems.isEmpty) {
        SnackBarHelper.showErrorSnackBar('Your cart is empty');
        return;
      }
      ...
      if (phoneController.text.isEmpty ||
          ...
          ditleController.text.isEmpty ||
          cityController.text.isEmpty) {
        SnackBarHelper.showErrorSnackBar('Please fill shipping address');
        return;
      }
  ...
  submitOrder(BuildContext context) async {
    if (selectedPaymentOption == 'الدفع نقداً') {
      //'cod''خدمات مصرفية'
      addOrder(context);
    } else {
  ...
  void retrieveSavedAddress() {
    ...
    ditleController.text = box.read(DITLE_KEY) ?? '';
    ...
    try {
      Map<String, dynamic> paymentData = {
        ...
        "amount": getGrandTotal() *
            100, //TODO: should complete amount grand total - its done
        "currency": "usd",
